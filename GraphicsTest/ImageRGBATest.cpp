...
{
	EXPECT_EQ(ColorRGBA<float>::Black(), ImageRGBA<float>::Black(1, 1).getColor(0, 0));
	EXPECT_EQ(ColorRGBA<float>::White(), ImageRGBA<float>::White(1, 1).getColor(0, 0));

}

TEST(ImageRGBATest, TestWritePNG)
{
	auto image = ImageRGBA<unsigned char>::Red(32, 32);
	//buffer.setColor(1, 0, ColorRGBA<unsigned char>(255, 0, 0, 0));
	image.save("../TestFile/IO/Image/testWrite.png", ImageFileFormat::PNG );
	const ImageRGBA<unsigned char> actual("../TestFile/IO/Image/testExpected.png");
...
TEST(ImageRGBATest, TestWriteBMP)
{
	...
	auto actual = ImageRGBA<unsigned char>::Red(32, 32);
...
{
	auto image = ImageRGBA<unsigned char>::Red(12, 12);
