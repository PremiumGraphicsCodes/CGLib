...
void FluidSample::setup()
{
	...
	Box<float> box(Vector3d<float>(0.0f, 0.0f, 0.0f), Vector3d<float>(100.0f, 100.0f, 10.0f));
	fluid = std::make_unique<Fluid>(box, 1.0f, constant);
	world.add(fluid.get());
	world.setExternalForce(Vector3d<float>(0.0, -9.8f, 0.0));
	Box<float> boundary( Vector3d<float>(-10.0, 0.0f, 0.0 ), Vector3d<float>(1000.0, 1000.0, 100.0 ));
