...
{
	{
		...
		//rigid1->getBody()->applyForce(btVector3(100.0f, 0.0f, 0.0f), btVector3(0.5f, 0.0f, 0.0f));
		rigid1->getBody()->setAngularVelocity(btVector3(1.0f, 0.0f, 0.0f));
		world.add(rigid1.get());
	}
	{
		rigid2 = std::make_unique<BulletRigid>(Vector3d<float>(0.5f, 5.0f, 0.5f), Vector3d<float>(2.0f, 0.0f, 0.0f), 0.1f);
		world.add(rigid2.get());
	}
	/*
	{
		const Box<float> box(Vector3d<float>(0.9f, 4.0f, 0.0f), Vector3d<float>(1.9f, 5.0f, 1.0f));
		rigid3 = std::make_unique<BulletRigid>(box, 1.0f);
	...
	*/
	...
	world.setExternalForce(Vector3d<float>(0, 0, 0));
