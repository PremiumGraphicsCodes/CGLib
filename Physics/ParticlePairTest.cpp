...
TEST(ParticlePairTest, TestIsValid)
{
	...
	{
		const auto p = std::make_shared< Particle<GeomType> >();
		const ParticlePair<GeomType> pair(p, p);
		EXPECT_FALSE(pair.isValid());
	}
	
	{
		const auto p1 = std::make_shared< Particle<GeomType> >();
		const auto p2 = std::make_shared< Particle<GeomType> >();
		const ParticlePair<GeomType> pair(p1, p2);
...
{
	const auto p1 = std::make_shared< Particle<GeomType> >( Vector3d<GeomType>( 0, 0, 0 ));
	const auto p2 = std::make_shared< Particle<GeomType> >( Vector3d<GeomType>( 1, 1, 1 ));
...
{
	const auto p1 = std::make_shared< Particle<GeomType> >(Vector3d<GeomType>(1, 2, 3));
	const auto p2 = std::make_shared< Particle<GeomType> >(Vector3d<GeomType>(0, 0, 0));
