...
public:
	...
	void createPairs(ParticleSPtrVector<float> particles, const GeomType effectLength) {
		if (particles.empty()) {
			return;
		}
		...
		std::vector<ParticleSPtrVector<float>::const_iterator> iters;
...
private:
	...
	ParticlePairVector search1(const ParticleSPtrVector<float>& particles, ParticleSPtrVector<float>::const_iterator startIter, ParticleSPtrVector<float>::const_iterator endIter, const float effectLengthSquared) {
		ParticlePairVector pairs;
	...
	ParticlePairVector search2(const ParticleSPtrVector<float>& particles, ParticleSPtrVector<float>::const_iterator startIter, ParticleSPtrVector<float>::const_iterator endIter, const float effectLengthSquared) {
		ParticlePairVector pairs;

		std::vector<ParticleSPtrVector<float>::const_iterator> yIter(4, startIter);
		...
		for (ParticleSPtrVector<float>::const_iterator xIter = startIter; xIter != endIter; ++xIter) {
